...
namespace AcmeWidgetCo\Infrastructure\DI;
...
class DIContainer implements ContainerInterface
{
     ...
     * @throws NotFoundExceptionInterface|ReflectionException
     */
    private function resolveDependency(ReflectionParameter $param, string $name, ReflectionType $type, array $arguments, array $parameters): mixed
    {
        if (isset($parameters[$name])) {
            return $parameters[$name];
        }

        $argumentDependency = $arguments[$name] ?? null;
        if ($argumentDependency !== null) {
            if (!is_array($argumentDependency) && !is_callable($argumentDependency) && !is_string($argumentDependency)) {
                throw new InvalidArgumentException(sprintf('Dependency configuration for %s must be an array, callable, or string; received %s.', $name, gettype($argumentDependency)));
            }
            return $this->handleArgumentDependency($argumentDependency);
        }
